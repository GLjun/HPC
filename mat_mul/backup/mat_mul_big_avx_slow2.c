		...
		for(ir = 0; ir < mc; ir += mr)
		{
			...
			for(i = 0;i < mr; i++)
			{
				...
				for(pi = 0; pi < nr; pi ++)
				{
					...
					p = &AB[si + i];
					pBC = &BP[sj];
					ar = _mm256_set_pd(*(p+3*mr), *(p+2*mr), *(p+mr), *(p));
					bc = _mm256_load_pd(pBC);
					sum0 = _mm256_fmadd_pd(ar, bc, ze);

					ar = _mm256_set_pd(*(p+7*mr), *(p+6*mr), *(p+5*mr), *(p+4*mr));
					bc = _mm256_load_pd(pBC+4);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);
					
					ar = _mm256_set_pd(*(p+11*mr), *(p+10*mr), *(p+9*mr), *(p+8*mr));
					bc = _mm256_load_pd(pBC+8);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+15*mr), *(p+14*mr), *(p+13*mr), *(p+12*mr));
					bc = _mm256_load_pd(pBC+12);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+19*mr), *(p+18*mr), *(p+17*mr), *(p+16*mr));
					bc = _mm256_load_pd(pBC+16);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+23*mr), *(p+22*mr), *(p+21*mr), *(p+20*mr));
					bc = _mm256_load_pd(pBC+20);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+27*mr), *(p+26*mr), *(p+25*mr), *(p+24*mr));
					bc = _mm256_load_pd(pBC+24);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+31*mr), *(p+30*mr), *(p+29*mr), *(p+28*mr));
					bc = _mm256_load_pd(pBC+28);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+35*mr), *(p+34*mr), *(p+33*mr), *(p+32*mr));
					bc = _mm256_load_pd(pBC+32);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+39*mr), *(p+38*mr), *(p+37*mr), *(p+36*mr));
					bc = _mm256_load_pd(pBC+36);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+43*mr), *(p+42*mr), *(p+41*mr), *(p+40*mr));
					bc = _mm256_load_pd(pBC+40);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+47*mr), *(p+46*mr), *(p+45*mr), *(p+44*mr));
					bc = _mm256_load_pd(pBC+44);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+51*mr), *(p+50*mr), *(p+49*mr), *(p+48*mr));
					bc = _mm256_load_pd(pBC+48);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+55*mr), *(p+54*mr), *(p+53*mr), *(p+52*mr));
					bc = _mm256_load_pd(pBC+52);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);

					ar = _mm256_set_pd(*(p+59*mr), *(p+58*mr), *(p+57*mr), *(p+56*mr));
					bc = _mm256_load_pd(pBC+56);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);
					
					ar = _mm256_set_pd(*(p+63*mr), *(p+62*mr), *(p+61*mr), *(p+60*mr));
					bc = _mm256_load_pd(pBC+60);
					sum0 = _mm256_fmadd_pd(ar, bc, sum0);
